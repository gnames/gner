...
`)
	res := token.Tokenize(text, wrapToken)
	fmt.Println()
	assert.Equal("d", string(res[0].Cleaned()))
	assert.Equal("Family", string(res[1].Cleaned()))
	assert.Equal("(I)", string(res[3].Cleaned()))
	assert.Equal("Blaniulus", string(res[4].Cleaned()))
}

func TestWideSpace(t *testing.T) {
	assert := assert.New(t)
	text := []rune(`d　Family　Blaniulidae
        (I)　Blaniulus　guttulatus`)
	res := token.Tokenize(text, wrapToken)
	fmt.Println()
	assert.Equal("d", string(res[0].Cleaned()))
	assert.Equal("Family", string(res[1].Cleaned()))
